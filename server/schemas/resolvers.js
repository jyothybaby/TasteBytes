  ...
  Mutation: {
    ...
    saveGrocery: async (parent, { groceryData }, context) => {
      if (context.user) {
        ...
        const updatedUser = await User.findByIdAndUpdate(
          { _id: context.user._id },
          { $addToSet: { savedGroceries: { $each: groceryData} } },
          {new:true}
        );
        return updatedUser;
      }
    ...
    saveInventory: async (parent, { inventoryData }, context) => {
      if (context.user) {
        const updatedUser = await User.findByIdAndUpdate(
          { _id: context.user._id },
          { 
            $addToSet: { savedInventories: { $each: inventoryData} },
            $pull: {savedGroceries: { $in: inventoryData}}
          },
          { new: true }
        );
        return updatedUser;
    ...
    removeInventory: async (parent, { inventoryData }, context) => {
      if (context.user) {
        const updatedUser = await User.findByIdAndUpdate(
          { _id: context.user._id },
          { 
            $pull: {savedInventories: { $in: inventoryData}}
          },
          { new: true }
        );
        return updatedUser;
